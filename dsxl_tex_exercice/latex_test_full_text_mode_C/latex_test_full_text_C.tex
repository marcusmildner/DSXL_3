...
ytick={-8.0,-7.0,...,22.0},]
\clip(-5.17031,-8.06760180995476) rectangle (8.9,22.2);
\draw[line width=2.pt,smooth,samples=100,domain=-3:4] plot(\x,{-1*(\x)^(2.0) +2*(\x)+12});
%\draw[line width=2.pt,smooth,samples=100,domain=-3.0:4.0] plot(\x,{(\x)^(2.0)-2*(\x)-5});
\begin{scriptsize}
...
\correction{ %début correction 
 ...
 Les antécédents de $7$ sont (valeurs approchées) $3.45$ et $-1.45$ \points{0.25} %en pourcentage
 } % fin correction 
...
$f(x) = -1$  $ \mbox{ pour } x \approx 4.74 \mbox{ et } x \approx -2.74\mbox{.}$ \points{0.25} %en pourcentage
} % fin correction 
...
$f(x) = -5$ $ \mbox{ pour } x \approx 5.24 \mbox{ et } x \approx -3.24\mbox{.}$ \points{0.25} %en pourcentage
} % fin correction 
...
Par lecture graphique on trouve $S =[ -1.65\,;\,3.65 ]$ \points{0.25} %en pourcentage
} % fin correction 
...
On sait maintenant, en plus, que f est définie par $f(x) =- x^{2} + 2 x + 12$.
} % fin énoncé 
...
\begin{eqnarray*}
 f(0) &=&12\\
  f(-1) &=& 2 \left(-1\right) - \left(-1\right)^{2} + 12 = 9\\
    f(\sqrt{2}) &=&- \left(\sqrt{2}\right)^{2} + 2 \sqrt{2} + 12 = 2 \sqrt{2} + 10
\end{eqnarray*}
\points{0.75} %en pourcentage
} % fin correction 
...
Montrer que $f(x) =13 - \left(x - 1\right)^{2}$. 
} % fin énoncé 

\correction{ %début correction 

\begin{eqnarray*}
 13 - \left(x - 1\right)^{2}  &=& - x^{2} + 2 x + 12 \\
  &=& - x^{2} + 2 x + 12\\
  &=& f(x) 
\end{eqnarray*}
\points{0.5} %en pourcentage
...
$$
\begin{array}{lll}  f(x) &=& 5 \\ 8 - \left(x - 1\right)^{2} &=&0 \\ - \left(x - 1 + 2 \sqrt{2}\right) \left(x - 2 \sqrt{2} - 1\right) &=&0 \\ \end{array}
$$


Donc (propriété équation-produit), comme $1 - 2 \sqrt{2}\in D$ 
et $1 + 2 \sqrt{2}\in D$, on a l'ensemble des solutions 
$ S =\{ 1 - 2 \sqrt{2}\,;\,1 + 2 \sqrt{2} \} $\points{0.5} %en pourcentage

Il faut résoudre $f(x)=-2$ : 
$$
\begin{array}{lll}  f(x) &=& -2 \\ 15 - \left(x - 1\right)^{2} &=&0 \\ - \left(x - 1 + \sqrt{15}\right) \left(x - \sqrt{15} - 1\right) &=&0 \\ \end{array}
$$

Donc (propriété équation-produit), comme $1 - \sqrt{15}\in D$ 
et $1 + \sqrt{15}\not\in D$, on a l'ensemble des solutions 
$ S =\{1 - \sqrt{15}\}$\points{0.75} %en pourcentage
} % fin correction 
